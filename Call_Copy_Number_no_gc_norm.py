...
ID = args.ID
gcn= args.gcn
kat_json1 = args.mode1
kat_json2 = args.mode2
...
with open(kat_json1, "r+") as json:
    while True:
        line = json.readline().strip()
        regex = re.search(r'\"global_maxima\"',line)
        if regex != None:
            line = json.readline().strip()
            kmer_maximum = re.search(r'\"freq\"\:[\s]+([0-9]+)',line)
            #Adding 1 here because the json file is 0 based indexing
            read_mode_1 = int(kmer_maximum.group(1)) + 1
        if line == "":
            break

with open(kat_json2, "r+") as json:
    while True:
        line = json.readline().strip()
        regex = re.search(r'\"global_maxima\"',line)
        if regex != None:
            ...
            kmer_maximum = re.search(r'\"freq\"\:[\s]+([0-9]+)',line)
            #Adding 1 here because the json file is 0 based indexing
            read_mode_2 = int(kmer_maximum.group(1)) + 1
        if line == "":
            break
